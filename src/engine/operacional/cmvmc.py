...
def cmvmc_anual(
    ...
    # Bottom-up para todos os anos: cup_mpsc = cip_unitario × mp_fraction × factor.
    # 2024: factor=1.0. A DR 2024 está pinned a dr_2024_real — este valor só
    # afecta os cálculos de stock do balanço.
    cips = prod_mod._cip_unitarios(a)
    mp_pct = prod_mod._mp_fraction_per_produto(a)
    ...
    peso_total = sum(
        cips[p] * mp_pct[p] * qtd_2024.get(p, 0.0)
        for p in PRODUTOS
    )

    cmvmc_prod_2024_total = sum(
        cips[p] * mp_pct[p] * qtd_2024.get(p, 0.0)
        for p in PRODUTOS
    )
    ...
    for y in ALL_YEARS:
        ...
        for p in PRODUTOS:
            ...
            if y == 2024:
                peso_p = (
                    (cips[p] * mp_pct[p] * qtd_2024.get(p, 0.0)) / peso_total
                    if peso_total > 0
                    else 0.0
                )
